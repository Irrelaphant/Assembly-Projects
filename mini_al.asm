...
start:
	...
	xchg eax,edi
	invoke alcMakeContextCurrent,edi

	; Generate 1 source for playback.
	invoke alGenSources,1,ebp
	...
	; Start playback.
	invoke uFMOD_OALPlaySong,OFFSET xm,xm_length,XM_MEMORY,[ebp]
	test eax,eax
	jz oal_error
	...
	; Stop playback.
	invoke uFMOD_OALPlaySong,ebx,ebx,ebx,ebx

cleanup:
	; Release the current context and destroy it (the source gets destroyed as well).
	invoke alcMakeContextCurrent,ebx
	invoke alcDestroyContext,edi

	; Close the actual device.
	invoke alcCloseDevice,esi
